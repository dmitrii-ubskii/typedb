...
async fn attribute_has_value(context: &mut Context, var: params::Var, value: params::Value) {
    ...
    with_read_tx!(context, |tx| {
        ...
        assert_eq!(attribute.get_value(tx.snapshot.as_ref(), &tx.thing_manager, StorageCounters::DISABLED).unwrap(), value);
    });
}
...
pub fn get_attribute_by_value(
    ...
    with_read_tx!(context, |tx| {
        ...
        tx.thing_manager.get_attribute_with_value(tx.snapshot.as_ref(), attribute_type, value, StorageCounters::DISABLED)
    })
}
...
async fn attribute_is_deleted(context: &mut Context, var: params::Var, is_deleted: params::Boolean) {
    ...
    let get = with_read_tx!(context, |tx| {
        ...
        tx.thing_manager.get_attribute_with_value(tx.snapshot.as_ref(), attribute_type, value, StorageCounters::DISABLED).unwrap()
    });
...
async fn delete_attributes_of_type(context: &mut Context, type_label: params::Label) {
    with_write_tx!(context, |tx| {
        ...
        while let Some(attribute) = attribute_iterator.next() {
            attribute.unwrap().delete(Arc::get_mut(&mut tx.snapshot).unwrap(), &tx.thing_manager, StorageCounters::DISABLED).unwrap();
        }
    })
}
