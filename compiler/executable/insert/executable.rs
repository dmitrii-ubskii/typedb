...
pub fn compile(
    ...
    add_role_players(constraints, type_annotations, &variable_positions, variable_registry, &mut connection_inserts)?;
...
fn add_inserted_concepts(
    ...
    let concept_instructions_vec = concept_instructions.into_values()
        .sorted_by(|a, b| a.inserted_position().cmp(b.inserted_position()))
        .collect();
