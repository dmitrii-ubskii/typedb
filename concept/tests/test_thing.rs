...
fn get_has_reverse_in_range() {
    ...
    let mut snapshot: WriteSnapshot<WALClient> = storage.clone().open_snapshot_write();
    {
        ...
        let age_10 = thing_manager.create_attribute(&mut snapshot, age_type, Value::Long(age_value_10)).unwrap();
        let age_11 = thing_manager.create_attribute(&mut snapshot, age_type, Value::Long(age_value_11)).unwrap();
