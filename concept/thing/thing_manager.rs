...
impl ThingManager {
    ...
    pub(crate) fn get_relations_player_role(
        ...
        self.get_relations_roles(snapshot, player, storage_counters).filter_map::<Result<(Relation, u64), _>, _>(move |item| match item {
    ...
    pub fn get_attributes_in_range<'a>(
        ...
        let Some((value_lower_bound, value_upper_bound)) = Self::get_value_range(&attribute_value_type, value_range) else {
            return Ok(AttributeIterator::new_empty());
        };
        let start_attribute_vertex_bound = self.get_attribute_vertex_prefix_lower_bound(attribute_type.vertex().type_id_(), &attribute_value_type, value_lower_bound);
        let end_attribute_vertex_bound = self.get_attribute_vertex_prefix_upper_bound(attribute_type.vertex().type_id_(), &attribute_value_type, value_upper_bound);
    ...
    fn get_value_lower_bound<'a>(
        ...
        Some(range.start_bound().map(|value| {
            if *expected_value_type != start_value_type {
                value.as_reference().approximate_cast_lower_bound(&expected_value_type)
    ...
    fn get_value_upper_bound<'a>(
        ...
        Some(range.end_bound().map(|value| {
            if *expected_value_type != end_value_type {
                value.as_reference().approximate_cast_upper_bound(&expected_value_type)
    ...
    pub(crate) fn get_has_from_thing_unordered<'a>(
        ...
        let range_start = match attribute_type_range_hint.start_bound() {
            Bound::Included(attribute_type) => {
                ...
                self.get_has_from_thing_to_type_unordered_start_bound(owner, attribute_type.vertex().type_id_(), &attribute_value_type, lower_bound)
            },
            Bound::Excluded(attribute_type) => {
                ...
                self.get_has_from_thing_to_type_unordered_start_bound(owner, next_attribute_id, &attribute_value_type, lower_bound)
            },
            Bound::Unbounded => {
        ...
        let range_end =
            match attribute_type_range_hint.end_bound() {
                Bound::Included(attribute_type) => {
                    ...
                    self.get_has_from_thing_to_type_unordered_end_bound(owner, attribute_type.vertex().type_id_(), &attribute_value_type, upper_bound)
                },
                Bound::Excluded(attribute_type) => {
                    let attribute_value_type = match attribute_type.get_value_type_without_source(snapshot, self.type_manager())? {
                        None => return Ok(HasIterator::new_empty()),
                    ...
                    self.get_has_from_thing_to_type_unordered_end_bound(owner, prev_attribute_id, &attribute_value_type, upper_bound)
                },
                Bound::Unbounded => {
                    RangeEnd::EndPrefixInclusive(ThingEdgeHas::prefix_from_object_to_type(owner.vertex(), TypeID::MAX).resize_to())
                }
            };
        let key_range = KeyRange::new(range_start, range_end, ThingEdgeHas::FIXED_WIDTH_ENCODING);
    ...
    pub(crate) fn get_has_from_thing_to_type_unordered<'a>(
        ...
        let has_start_bound = self.get_has_from_thing_to_type_unordered_start_bound(owner, attribute_type.vertex().type_id_(), &attribute_value_type, value_lower_bound);
        let has_end_bound = self.get_has_from_thing_to_type_unordered_end_bound(owner, attribute_type.vertex().type_id_(), &attribute_value_type, value_upper_bound);
