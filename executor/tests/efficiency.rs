...
fn setup_database(storage: &mut Arc<MVCCStorage<WALClient>>) {
    ...
    */
    ...
    let name_abby = thing_manager.create_attribute(&mut snapshot, name_type, Value::String(Cow::Borrowed("abby"))).unwrap();
    let name_bolston = thing_manager.create_attribute(&mut snapshot, name_type, Value::String(Cow::Borrowed("bolston"))).unwrap();
    let name_uninlineable = thing_manager.create_attribute(&mut snapshot, name_type, Value::String(Cow::Borrowed("longy-mc-long-face-uninlineable"))).unwrap();
    let name_willa = thing_manager.create_attribute(&mut snapshot, name_type, Value::String(Cow::Borrowed("willa"))).unwrap();
