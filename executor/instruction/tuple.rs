...
use concept::{
    ...
    thing::{
        has::Has,
        relation::IndexedRelationPlayers,
    },
...
use concept::thing::relation::{Links, Relation};
...
fn tuple_owner_attribute_to_owner_attribute<'a>(tuple: &'a Tuple<'a>) -> (Object, &'a Attribute) {
    let owner = tuple.values().get(0)
        .expect("Reverse tuple mapping missing owner")
        .get_thing().map(|thing| thing.as_object()).unwrap_or_else(|| Object::MIN);
    let attribute = tuple.values().get(1)
        .expect("Reverse tuple mapping missing attribute")
        .get_thing().map(|thing| thing.as_attribute())
...
fn tuple_attribute_owner_to_attribute_owner<'a>(tuple: &'a Tuple<'_>) -> (&'a Attribute, Object) {
    let attribute = tuple.values().get(0)
        .expect("Reverse tuple mapping missing attribute").get_thing().map(|thing| thing.as_attribute())
        .unwrap_or_else(|| &concept::thing::attribute::MIN_STATIC);
    let owner = tuple.values().get(1)
        .expect("Reverse tuple mapping missing owner").get_thing().map(|thing| thing.as_object())
...
pub(crate) fn tuple_relation_player_role_to_links_canonical(tuple: &Tuple<'_>, fixed_bounds: &FixedLinksBounds) -> Links {
...
fn tuple_relation_player_role_to_relation_player_role(tuple: &Tuple<'_>) -> (Relation, Object, RoleType) {
    let relation = tuple.values().get(0)
        .expect("Reverse tuple mapping missing relation")
        .get_thing().map(|thing| thing.as_relation())
        .unwrap_or_else(|| Relation::MIN);
    let player = tuple.values().get(1)
        .expect("Reverse tuple mapping missing player")
        .get_thing().map(|thing| thing.as_object())
        .unwrap_or_else(|| Object::MIN);
    let role = tuple.values().get(2)
        .expect("Reverse tuple mapping missing role type")
        .get_type().map(|type_| type_.as_role_type())
...
pub(crate) fn tuple_player_relation_role_to_links_canonical(tuple: &Tuple<'_>, fixed_bounds: &FixedLinksBounds) -> Links {
...
fn tuple_player_relation_role_to_player_relation_role(tuple: &Tuple<'_>) -> (Object, Relation, RoleType) {
    let player = tuple.values().get(0)
        .expect("Reverse tuple mapping missing player")
        .get_thing().map(|thing| thing.as_object())
        .unwrap_or_else(|| Object::MIN);
    let relation = tuple.values().get(1)
        .expect("Reverse tuple mapping missing relation")
        .get_thing().map(|thing| thing.as_relation())
        .unwrap_or_else(|| Relation::MIN);
    let role = tuple.values().get(2)
...
pub(crate) fn tuple_role_relation_player_to_links_canonical(tuple: &Tuple<'_>, fixed_bounds: &FixedLinksBounds) -> Links {
