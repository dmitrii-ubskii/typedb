...
use crate::instruction::{
    ...
    BinaryEdgeExecutor, FilterMapUnchangedFn, MapToTupleFn,
};

type ExecutorResult<T> = Result<T, Box<ConceptReadError>>;
pub(super) trait ExecutorIteratorUnbound<Executor: BinaryEdgeExecutor>:
...
pub(super) trait ExecutorIteratorUnboundInverted<Executor: BinaryEdgeExecutor>:
    Iterator<Item = ExecutorResult<Executor::Element>> + Sized
{
...
pub(super) trait ExecutorIteratorBoundFrom<Executor: BinaryEdgeExecutor>:
    Iterator<Item = ExecutorResult<Executor::Element>> + Sized
{
    ...
    ($($executor:ty : $iter:ty => $variant:ident,)*) => {
        $(
            impl ExecutorIteratorUnbound<$executor> for $iter {
                fn unbound_into_tuple_iterator(self, filter_for_row: Box<FilterMapUnchangedFn<<$executor as BinaryEdgeExecutor>::Element>>, to_tuple: MapToTupleFn<<$executor as BinaryEdgeExecutor>::Element>, tuple_positions: TuplePositions, variable_modes: &VariableModes) -> TupleIterator {
    ...
    ($($executor:ty : $iter:ty => $variant:ident,)*) => {
        $(
            impl ExecutorIteratorUnboundInverted<$executor> for $iter {
                fn unbound_inverted_into_tuple_iterator(self, filter_for_row: Box<FilterMapUnchangedFn<<$executor as BinaryEdgeExecutor>::Element>>, to_tuple: MapToTupleFn<<$executor as BinaryEdgeExecutor>::Element>, tuple_positions: TuplePositions, variable_modes: &VariableModes) -> TupleIterator {
                    let checked_tupled = self.filter_map(filter_for_row).map(to_tuple);
                    TupleIterator::$variant(SortedTupleIterator::new(checked_tupled, tuple_positions, variable_modes))
                }
            }
    ...
    ($($executor:ty : $iter:ty => $variant:ident,)*) => {
        $(
            impl ExecutorIteratorBoundFrom<$executor> for $iter {
                fn bound_from_into_tuple_iterator(self, filter_for_row: Box<FilterMapUnchangedFn<<$executor as BinaryEdgeExecutor>::Element>>, to_tuple: MapToTupleFn<<$executor as BinaryEdgeExecutor>::Element>, tuple_positions: TuplePositions, variable_modes: &VariableModes) -> TupleIterator {
                    let checked_tupled = self.filter_map(filter_for_row).map(to_tuple);
                    TupleIterator::$variant(SortedTupleIterator::new(checked_tupled, tuple_positions, variable_modes))
                }
            }
...
impl<T: BinaryEdgeExecutor> ExecutorIteratorUnboundInverted<T> for UnreachableIteratorType<T::Element> {
    fn unbound_inverted_into_tuple_iterator(
        self,
