...
use encoding::graph::type_::vertex::{TypeID, TypeVertexEncoding};
...
use crate::{
    instruction::{
        ...
        ,
        FilterFn,
        iterator::{SortedTupleIterator, TupleIterator}, tuple::{Tuple, TuplePositions, TupleResult},
    },
...
use crate::instruction::tuple::{TupleOrderingFn, unsafe_compare_result_tuple};

pub(crate) type IndexedRelationTupleIteratorSingle = IndexedRelationTupleIterator<IndexedRelationsIterator>;
pub(crate) type IndexedRelationTupleIteratorMerged = KMergeBy<IndexedRelationTupleIterator<IndexedRelationsIterator>, TupleOrderingFn>;
...
pub(super) const EXTRACT_RELATION: IndexedRelationValueExtractor =
    |(indexed, _)| VariableValue::Thing(Thing::Relation(Relation::new(ObjectVertex::build_relation(indexed.2, indexed.3))));
...
pub(crate) type IndexedRelationOrderingFn = for<'a, 'b> fn(
    (&'a Result<(IndexedRelationPlayers, u64), Box<ConceptReadError>>, &'b Result<(IndexedRelationPlayers, u64), Box<ConceptReadError>>),
) -> Ordering;
        ...
        match self.iterate_mode {
                    ...
                    |start_player| {
                        for relation_type in self.relation_to_player_start_types.keys() {
                            let iterator = start_player
                                    .get_indexed_relations(snapshot, thing_manager, relation_type.as_relation_type(), storage_counters.clone())
            ...
            IndexedRelationIterateMode::BoundStart => {
                ...
                if self.relation_to_player_start_types.len() == 1 {
                    ...
                    let as_tuples = IndexedRelationTupleIterator::new(
                        ...
                        self.tuple_positions.clone(),
                        component_ordering,
                        FixedIndexedRelationBounds::new(Some(relation_type.vertex().type_id_()), None, Some(start_player), None),
                    );
                    Ok(TupleIterator::IndexedRelationsSingle(SortedTupleIterator::new(
                        as_tuples,
                        self.tuple_positions.clone(),
            ...
            IndexedRelationIterateMode::BoundStartBoundEnd => {
                ...
                if self.relation_to_player_start_types.len() == 1 {
                    let relation_type = self.relation_to_player_start_types.keys().next().unwrap().as_relation_type();
                    let iterator = start_player
                        .get_indexed_relations_with_player(snapshot, thing_manager, end_player, relation_type, storage_counters)
                        .expect("Relation index should be available");
                    let as_tuples = IndexedRelationTupleIterator::new(
                        iterator,
                        filter_for_row,
            ...
            IndexedRelationIterateMode::BoundStartBoundEndBoundRelation => {
                ...
                let iterator = start_player
                    .get_indexed_relation_roles_with_player_and_relation(snapshot, thing_manager, end_player, relation, storage_counters)
                    .expect("Relation index should be available");
                let as_tuples = IndexedRelationTupleIterator::new(
                    iterator,
                    filter_for_row,
...
impl FixedIndexedRelationBounds {
    fn new(relation_type_id: Option<TypeID>, relation_id: Option<ObjectID>, from: Option<Object>, to: Option<Object>) -> Self {
            ...
            for<'a> LendingIterator<Item<'a>=Result<(IndexedRelationPlayers, u64), Box<ConceptReadError>>> +
{
    fn indexed_to_tuple(
        ...
        let tuple: [VariableValue<'static>; 5] = std::array::from_fn(|i| {
            let variable_at_position = self.tuple_positions.as_quintuple()[i].expect("Indexed tuple positions must be a quintuple");
            let source_component_index = self.component_ordering.iter().position(|var| *var == variable_at_position).unwrap();
            match source_component_index {
                0 => VariableValue::Thing(components.0.into()),
                1 => VariableValue::Thing(components.1.into()),
                2 => VariableValue::Thing(Thing::Relation(Relation::new(ObjectVertex::build_relation(components.2, components.3)))),
    ...
    fn tuple_to_indexed(&self, tuple: &Tuple<'_> ) -> IndexedRelationPlayers {
        ...
        let mut indexed: (Option<Object>, Option<Object>, Option<TypeID>, Option<ObjectID>, Option<RoleType>, Option<RoleType>) = (
        ...
        for (index, value) in tuple.values().iter().enumerate() {
            let variable_at_position = self.tuple_positions.as_quintuple()[index].expect("Indexed tuple positions must be a quintuple");
            let source_component_index = self.component_ordering.iter().position(|var| *var == variable_at_position).expect("Variable -> indexed component not found.");
            match source_component_index {
                0 => if indexed.0.is_none() {
                    indexed.0 = Some(value.get_thing().map(|thing| thing.as_object()).unwrap_or_else(|| Object::MIN))
                },
                1 => if indexed.1.is_none() {
                    indexed.1 = Some(value.get_thing().map(|thing| thing.as_object()).unwrap_or_else(|| Object::MIN))
                },
                2 => {
                    if indexed.2.is_none () {
                        indexed.2 = Some(value.get_thing().map(|thing| thing.as_relation().type_().vertex().type_id_()).unwrap_or_else(|| TypeID::MIN))
                    }
                    if indexed.3.is_none() {
                        indexed.3 = Some(value.get_thing().map(|thing| thing.as_relation().vertex().object_id()).unwrap_or_else(|| ObjectID::MIN))
                    }
                },
                3 => if indexed.4.is_none() {
                    indexed.4 = Some(value.get_type().map(|type_| type_.as_role_type()).unwrap_or_else(|| RoleType::MIN))
                },
                4 => if indexed.5.is_none() {
                    indexed.5 = Some(value.get_type().map(|type_| type_.as_role_type()).unwrap_or_else(|| RoleType::MIN))
                },
                _ => unreachable!("only 5 components exist"),
            }
        }
        (indexed.0.unwrap(), indexed.1.unwrap(), indexed.2.unwrap(), indexed.3.unwrap(), indexed.4.unwrap(), indexed.5.unwrap())
    }
}
...
fn compare_indexed_players(
    (left, right): (&Result<(IndexedRelationPlayers, u64), Box<ConceptReadError>>, &Result<(IndexedRelationPlayers, u64), Box<ConceptReadError>>),
) -> Ordering {
...
fn compare_indexed_players_inverted(
    ...
    ) = (left, right)
    {
        (end_1, start_1, rel_type_1, rel_id_1, start_role_1, end_role_1).cmp(
            &(end_2, start_2, rel_type_2, rel_id_2, start_role_2, end_role_2)
        )
    } else {
        // arbitrary
        Ordering::Equal
