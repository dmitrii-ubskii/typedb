...
pub fn translate_match<'a>(
    ...
    add_patterns(function_index, struct_index, &mut conjunction, &match_.patterns)?;
    Ok(builder)
}
...
pub(crate) fn add_patterns(
    ...
    patterns.iter().try_for_each(|pattern| match pattern {
        typeql::Pattern::Conjunction(nested) => {
            add_patterns(function_index, struct_index, conjunction, &nested.patterns)
        }
        typeql::Pattern::Disjunction(disjunction) => {
            add_disjunction(function_index, struct_index, conjunction, disjunction)
        }
        typeql::Pattern::Negation(negation) => add_negation(function_index, struct_index, conjunction, negation),
        typeql::Pattern::Optional(optional) => add_optional(function_index, struct_index, conjunction, optional),
        typeql::Pattern::Statement(statement) => add_statement(function_index, struct_index, conjunction, statement),
    })?;
    Ok(())
}
...
fn add_disjunction(
    ...
    disjunction.branches.iter().try_for_each(|branch| {
        ...
        add_patterns(function_index, struct_index, &mut conj, branch)
    })?;
    Ok(())
}
...
fn add_negation(
    ...
    add_patterns(function_index, struct_index, &mut negation_builder, &negation.patterns)
}
...
fn add_optional(
    ...
    add_patterns(function_index, struct_index, &mut optional_builder, &optional.patterns)?;
    Ok(())
}
